...
document.getElementById('upload-container').addEventListener('click', function() {
  // Activează inputul pentru încărcarea fișierelor
  document.getElementById('file-input').click();
});

// Ascultă evenimentul de schimbare a fișierului pentru a executa acțiunile necesare
document.getElementById('file-input').addEventListener('change', function(event) {
  // Obține fișierele selectate
  const files = event.target.files;
  
  // Aici poți executa orice acțiune dorești cu fișierele încărcate, cum ar fi afișarea lor, încărcarea lor pe server etc.
  // De exemplu, poți afișa numele fișierului în consolă
  for (let i = 0; i < files.length; i++) {
      console.log('Fișier încărcat:', files[i].name);
  }
});
